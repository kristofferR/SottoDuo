...
final class CaptureTests: XCTestCase {
    ...
    func testCaptureModelsAndGenerationRoundTripThroughContract() throws {
        ...
        let decoded = try SottoAPI.decodeWire(StartCaptureRequest.self, from: SottoAPI.encodeWire(request))
        ...
        XCTAssertEqual(try SottoAPI.decodeWire(StopCaptureRequest.self, from: SottoAPI.encodeWire(stop)).continuationID, stop.continuationID)
        ...
        let roundTrip = try SottoAPI.decodeWire(GenerationRecord.self, from: SottoAPI.encodeWire(record))
