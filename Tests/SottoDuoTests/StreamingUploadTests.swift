...
final class StreamingUploadTests: XCTestCase {
    private func client(blockUpgrade: Bool = false) throws -> ServerClient {
        guard let endpoint = ProcessInfo.processInfo.environment["SOTTODUO_STREAM_TEST_URL"] else {
            throw XCTSkip("Run Server/tests/fixtures/streaming-client-server.ts for the native streaming contract test.")
        }
        let configuration = URLSessionConfiguration.ephemeral
        if blockUpgrade { configuration.httpAdditionalHeaders = ["X-SottoDuo-Test-Block-Upgrade": "1"] }
        return try ServerClient(endpoint: endpoint, token: "sottoduo-native-streaming-test-token-2026",
