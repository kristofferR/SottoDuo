...
- **Check shortcut** consumes Sotto press/release/cancel/copy events for 30 seconds without recording or clipboard actions. A bounded, timestamped diagnostic log shows those events in an expandable section. A shared capture gate also blocks GUI microphone tests and pairing-button starts during the check. Finish, leaving settings, or hiding the window ends the check; a key still held when it ends must be released before recording is allowed. Only Sotto's namespaced commands are observed, never arbitrary typed keys.
