...
- Shared paginated history with transcript selection/copy.
...
Background-client service installation still uses the existing Linux integration. The GUI can configure an installed/running client but does not install or start that service automatically. Shortcut editing currently supports Sotto's standard Omarchy Lua bindings; other desktops and custom formats require their own shortcut setup. Audio playback, history deletion and draft persistence across application restarts are not yet implemented. The GUI is deployed on the Omarchy desktop; overlay verification uses synthetic state without opening a microphone.
