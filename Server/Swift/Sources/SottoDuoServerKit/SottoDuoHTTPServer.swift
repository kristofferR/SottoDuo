...
public enum SottoDuoHTTPServer {
    public static func run(configuration: ServerConfiguration) async throws {
        ...
        let app = Application(router: router,
            configuration: .init(address: .hostname(configuration.host, port: configuration.port), serverName: configuration.development ? "SottoDuo Dev" : "SottoDuo"))
        await service.start()
    ...
    public static func makeRouter(service: GenerationService, token: String? = nil) -> Router<BasicRequestContext> {
        ...
        router.post("/v1/generations/:id/audio/:kind") { request, context in
            ...
            let buffer = try await request.body.collect(upTo: SottoDuoAPI.maximumChunkBytes)
    ...
    private static func decode<T: Decodable>(_ type: T.Type, request: Request) async throws -> T {
        ...
        do { return try SottoDuoAPI.decoder().decode(type, from: Data(buffer.readableBytesView)) }
...
private struct ServerMiddleware: RouterMiddleware {
    ...
    func handle(_ request: Request, context: Context, next: (Request, Context) async throws -> Response) async throws -> Response {
        do {
            ...
            return try SottoDuoHTTPServer.json(APIErrorResponse(code: error.code, message: error.message), status: .init(code: error.status))
        } catch let error as HTTPError {
            return try SottoDuoHTTPServer.json(APIErrorResponse(code: "http_\(error.status.code)", message: error.body ?? error.status.reasonPhrase), status: error.status)
        } catch let error as any HTTPResponseError {
            return try SottoDuoHTTPServer.json(APIErrorResponse(code: "http_\(error.status.code)", message: error.status.reasonPhrase), status: error.status)
        } catch {
            ...
            context.logger.error("SottoDuo request failed", metadata: ["errorType": .string(String(describing: type(of: error)))])
            return try SottoDuoHTTPServer.json(APIErrorResponse(code: "internal_error", message: "The server could not complete this request."), status: .internalServerError)
        }
    }
