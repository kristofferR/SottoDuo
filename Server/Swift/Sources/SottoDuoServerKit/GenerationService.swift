...
public actor GenerationService {
    ...
    public init(configuration: ServerConfiguration, inference: NativeInference? = nil) throws {
        ...
        if files.fileExists(atPath: preferencesURL.path) {
            ...
            preferences = try SottoDuoAPI.decoder().decode(PreferencesSnapshot.self, from: Data(contentsOf: preferencesURL))
            if let error = preferences.preferences.validationError { throw ServiceError(500, "invalid_preferences", error) }
        } else {
            preferences = PreferencesSnapshot()
            try SottoDuoAPI.encoder().encode(preferences).write(to: preferencesURL, options: .atomic)
        }
        for child in try files.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil, options: .skipsHiddenFiles) {
            ...
            var record = try SottoDuoAPI.decoder().decode(GenerationRecord.self, from: Data(contentsOf: metadata))
            ...
            if !record.status.isTerminal {
                ...
                let recovered = try SottoDuoAPI.encoder().encode(record)
    ...
    public func updatePreferences(_ update: PreferencesSnapshot) throws -> PreferencesSnapshot {
        ...
        let data = try SottoDuoAPI.encoder().encode(next)
    ...
    public func appendAudio(_ id: UUID, kind: AudioKind, sequence: Int, format: AudioStreamFormat, data: Data) throws -> AudioChunkReceipt {
        ...
        guard !data.isEmpty, data.count <= SottoDuoAPI.maximumChunkBytes, data.count % (format.channels * 4) == 0 else {
            throw ServiceError(data.count > SottoDuoAPI.maximumChunkBytes ? 413 : 400, "invalid_chunk", "Audio chunks must contain complete float32 frames and fit within 1 MiB.")
        }
        ...
        guard duration <= Double(SottoDuoAPI.maximumRecordingSeconds) + 0.1, nextBytes <= 268_435_456 else {
    ...
    public func history(limit: Int, before: String?, source: String? = nil) throws -> GenerationPage {
        ...
        guard source == nil || source == "wispr-flow" || source == "sottoduo" else {
            throw ServiceError(400, "invalid_source", "Choose Wispr Flow or SottoDuo history.")
        }
        let selected = records.values.filter { record in
            switch source {
            case "wispr-flow": return record.importedSource?.provider == "wispr-flow"
            case "sottoduo": return record.importedSource == nil
            default: return true
            }
        }
    ...
    public func completeWisprFlowImport(_ id: UUID) throws -> WisprFlowImportResult {
        ...
        if let existingID = wisprFlowIndex[request.sourceID] {
            ...
            let metadata = try SottoDuoAPI.encoder().encode(record)
    ...
    private func save(_ record: GenerationRecord) throws {
        let data = try SottoDuoAPI.encoder().encode(record)
