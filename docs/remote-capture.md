...
- Preparation has a 3-second deadline. If discovery becomes obsolete during preparation, cancel the admission. A client may re-resolve **once** to its next eligible input within its original 3-second activation budget, using a fresh request ID/secret, only before acknowledged recording; otherwise report failure. No unbounded connection/retry loop or implicit Bluetooth pairing.
- The owner lease lasts 5 seconds. It begins at admission; send heartbeats while preparing, recording and draining. A 250 ms watchdog aborts expired leases, observed source unavailability and recordings reaching 180 seconds. An already-expired lease cannot be renewed, even before the watchdog runs.
