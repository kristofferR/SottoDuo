...
final class ConfigurationFileTests: XCTestCase {
    func testCreatesPrivateConfigOnlyOnce() async throws {
        ...
        let initial = SottoDuoConfiguration(holdKey: "fn", launchAtLogin: true, djiMicButtonEnabled: true,
        ...
        XCTAssertEqual(try JSONDecoder().decode(SottoDuoConfiguration.self, from: Data(contentsOf: location.url)), initial)
    ...
    func testKnownMissingFieldsDefaultButExplicitNullAndInvalidTypesDoNot() throws {
        ...
        for json in ["{}", #"{"microphones":{}}"#] {
            XCTAssertEqual(try decoder.decode(SottoDuoConfiguration.self, from: Data(json.utf8)), .default)
        }
        for json in [
    ...
    func testStrictMicrophoneValidationDoesNotSilentlyDiscardBadManualEdits() throws {
        ...
        for microphones in invalidMicrophones {
            ...
            XCTAssertThrowsError(try JSONDecoder().decode(SottoDuoConfiguration.self, from: data), microphones)
        }
        let valid = Data(#"{"microphones":{"profiles":[{"id":"desk","priority":[{"uid":"usb:desk"}]}],"selection":{"mode":"systemDefault"}}}"#.utf8)
        let decoded = try JSONDecoder().decode(SottoDuoConfiguration.self, from: valid)
    ...
    func testInvalidExistingFileIsUntouchedByLoadAndUpdate() async throws {
        ...
        var changed = SottoDuoConfiguration.default
        changed.holdKey = "fn"
    ...
    func testInvalidAppUpdateDoesNotChangeConfigAndExternalAtomicReplacementIsReadable() async throws {
        ...
        XCTAssertEqual(try Data(contentsOf: location.url), original)
        let replacement = SottoDuoConfiguration(holdKey: "fn", launchAtLogin: true)
    ...
    private func fixture() throws -> (root: URL, url: URL) {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("SottoDuo-config-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        return (root, root.appendingPathComponent("SottoDuo Dev", isDirectory: true).appendingPathComponent("config.json"))
    }
    ...
    private func assertInvalid(_ result: Result<SottoDuoConfiguration, ConfigurationFileError>,
    ...
    private func assertUnsafe(_ result: Result<SottoDuoConfiguration, ConfigurationFileError>,
                              file: StaticString = #filePath, line: UInt = #line) {
