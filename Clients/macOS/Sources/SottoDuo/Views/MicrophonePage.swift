    ...
    private func profileToolbar(_ profile: MicrophoneProfile) -> some View {
        HStack(spacing: 10) {
            ...
            Menu {
                ...
                SottoDuoControlIcon(systemName: "ellipsis")
            }
        ...
        .background {
            if isDragged {
                RoundedRectangle(cornerRadius: 8)
                    .fill(SottoDuoPalette.tint)
                    .overlay {
                        RoundedRectangle(cornerRadius: 8)
                            .strokeBorder(SottoDuoPalette.muted.opacity(0.55), style: StrokeStyle(lineWidth: 1, dash: [4, 4]))
                    }
            }
        }
        .background {
           ...
           let sourceFrame = priorityRowFrames[item.id] {
            ...
            ZStack(alignment: .topLeading) {
                priorityRowContent(device, position: source, connected: connected[device.id])
                    ...
                    .background(SottoDuoPalette.surface, in: RoundedRectangle(cornerRadius: 8))
                    .overlay {
                        RoundedRectangle(cornerRadius: 8)
                            .strokeBorder(SottoDuoPalette.accentInk.opacity(0.65), lineWidth: 1)
                    }
    ...
    private func deviceLabel(_ device: AudioInputDevice, available: Bool) -> some View {
        HStack(spacing: 10) {
            Image(systemName: device.transport.symbol)
                .font(.system(size: 15))
                .foregroundStyle(available ? SottoDuoPalette.accentInk : SottoDuoPalette.muted)
...
private struct MicrophoneReorderHandle: View {
    var body: some View {
        ...
        .foregroundStyle(SottoDuoPalette.muted)
        .frame(width: 28, height: 28)
        .background(SottoDuoPalette.surface.opacity(0.001))
        .contentShape(Rectangle())
        .accessibilityHidden(true)
    }
}
